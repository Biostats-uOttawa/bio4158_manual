...
The `vcd` `r emo::ji("package")` includes a `mosaic()` function useful to graphically display contingency tables:


```{r, fig.cap = "Mosaic plot of sex classes per age"}
library(vcd)
...
The mosaic plot from the `vcd` `r emo::ji("package")` can be shaded to show the categories that contribute most to the lack of independence:


```{r, fig.cap = "Mosaic plot of sex by age with colours"}
...
```{r, fig.cap = "Frequency of female and male sturgeon as a function of year and location"}
