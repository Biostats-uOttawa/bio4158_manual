...
```{r, ano-1, echo = params$echo_sol, class.source = "exer"}
...
```{r, ano-2, warning = FALSE, message = FALSE, fig.cap = "Distribution of sturgeon length per year", echo = params$echo_sol, class.source = "exer" }
...
```{r, fig.cap = "Boxplot of sturgeon length per year", echo = params$echo_sol, class.source = "exer" }
...
```{r, ano-3, fig.cap = "Diagnostic plots for a one-way ANOVA"}
...
```
```{r, ano-13, fig.cap = "Inter-annual differences in sturgeon length"}
par(mar = c(4, 7, 2, 1))
plot(TukeyHSD(aov(fklngth ~ myyear, data = dam10dat)), las = 2)
```

The confidence intervals, corrected for multiple tests by the Tukey method, are plotted for differences among years. Unfortunately, the labels are not all printed because they would overlap, but the order is the same as in the preceding table. The `multcomp` `r emo::ji("package")` can produce a better plot version, but requires a bit more code:

```{r, ano-14, fig.cap = "Inter-annual differences in sturgeon length"}
...
```{r, ano-15, fig.cap = "Inter-annual differences in sturgeon length"}
...
```{r, ano-16, fig.cap = "Diagnostic plots for the ANOVA of sturgeon length by year" }
