...
```{r, fig.cap = "Checking model asusmptions for regression of *logherp* as a function of *logarea*"}
...
```{block2, type="rmdcode"}
Rerun the above regression one more time, this time adding both inde- pendent variables into the model at once, such that `logherp ~ logarea + cpfor2` . What do you conclude?
```
...
```
 ...
 The help file for the `HH`  `r emo::ji("package")`package contains this clear passage about one of the indices of multicollinearity, the variance inflation factors:
...
Let’s have a look at the diagnostic plots for the final model. Here we use the `check_model()` function from the `performance` `r emo::ji("package")`.
```{r, echo = params$echo_sol, class.source = "exer", fig.cap = "Conditions d'application du modèle `model_poly2`"}
library(performance)
...
```{r, fig.cap = "Graphiques de résidus partiels du modèle `model_poly2`"}
...
```{r, echo = params$echo_sol, class.source = "exer", fig.cap = "Graphiques de résidus partiels du modèle `model_nopoly`"}
model_nopoly <- lm(
...
The following code, using the `simpleboot` `r emo::ji("package")` has been designed to be easily modifiable and will compute CI using empirical percentiles. Following this is an easier approach using the library boot that will calculate several different bootstrap confidence limits.
...
```{r, echo = FALSE, fig.cap = "Distribution of bootstrapped estimates for `logarea`"}
i <- 2
par(mfrow = c(1, 2))
lab <- colnames(tmyresults)[i]
x <- tmyresults[, i]
