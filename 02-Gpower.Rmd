...
important to use the correct one for each package. G*Power has an
...
```{r, gpower-1, fig.cap = "Post-hoc analysis with estimated effect size", out.width = "100%", echo = FALSE  }
knitr::include_graphics("images/gpower_1.png")
```

Similar analysis can be done in R. You first need to calculate the effect size `d` for a t-test comparing 2 means, and then use the `pwr.t.test()` function from the `pwr` `r emo::ji("package")`.
...
```{r, r-power, fig.cap = "Post-hoc analysis with estimated effect size in R"}
# load package pwr
library(pwr)
...
- The curve on the left, in red, corresponds to the expected distri- bution of the t-statistics when $H_0$ is true (*i.e.* when the two means are equal) given the sample size (18 per region) and the observed standard deviations. 
...
```{r, gpower-2, fig.cap = "Post-hoc analysis using an effect size different from the one estimated", out.width = "100%", echo = FALSE  }
...
```{r, gpower-3, fig.cap = "A priori analysis", out.width = "100%", echo = FALSE }
...
Black fly larvae (Diptera: Simuliidae) were sampled in February at the outlet of two lakes of the Eastern Townships (Lakes Orford and Lovering). The body length of each larva was measured. The data are in the file simulies.csv
The relationship between body length (L, in mm) and dry mass (M, in $\mu$g) for the dominant species (*P. mixtum/fuscum*) is:
...
2. Using the mean mass of *P. mixtum/fuscum* at Lovering as a reference and the standard deviations observed at each site, calculate the power of a two-tailed t test for independent means
  a) if the mass difference was 5 $\mu$g, $\alpha$=0.05, and that 100 larvae were sampled at each site
  b) if the mass difference was 20 $\mu$g, $\alpha$=0.05, and that 100 larvae were sampled at each site
  c) if the mass difference was 50 $\mu$g, $\alpha$=0.05, and that 100 larvae were sampled at each site
  d) comment on how changing the mass difference affects power.
3. Calculate the required sample size to detect, with a two-tailed t-test for independent means, a difference of 50 $\mu$g between the means
  a) with a power of 80% and $\alpha$=0.05
  b) with a power of 80% and $\alpha$=0.001
  c) with a power of 95% and $\alpha$=0.05
  d) comment on how changing power and $\alpha$ affects the required sample size.
4. Calculate the detectable effect size (`d`) with a two-tailed t-test on independent means, given the observed standard deviations
  ...
  d) comment on how changing sample sizes affect `d`.
5. Calculate the difference in average mass, in $\mu$g, that can be detected given your estimate of the minimum detectable effect size (`d`) at 4a, b, and c.
-->
